...
export class BinanceProvider implements MarketDataProvider {
  ...
  async fetchLatestPrice(symbol: string): Promise<number> {
    ...
    return this.retryRequest(async () => {
      ...
      if (isNaN(price)) {
        throw new Error(`Invalid price data from Binance: ${response.data.price}`);
      }
  ...
  private convertInterval(interval: string): string {
    ...
    const binanceInterval = mapping[normalized];
  ...
  /**
   * 解析K线数据
   * Binance API 返回格式：[timestamp, open, high, low, close, volume, ...]
   */
  ...
  private formatError(error: any): Error {
    if (error && typeof error === 'object') {
      if (error.response) {
        ...
        return new Error(`Binance API error (${status}): ${message}`);
      } else if (error.request) {
        // 网络错误
        const message = error.message || 'Network error';
        return new Error(`Binance API network error: ${message}`);
